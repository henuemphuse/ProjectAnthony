...
Unlike modern sandboxed packages that can leave stray configuration files behind on the host OS, Project Anthony can clean up after itself completely. Open the emergency terminal workspace or strike `Ctrl+Alt+X`, type **`u`** or `uninstall` at the option prompt, and confirm. From a living desktop the TUI asks for your `sudo` password, then a detached `systemd-run` unit finishes purge and getty restore so the TUI can exit. After it succeeds, the window asks **Exit to desktop?** — `y` closes it, `n` drops to a shell. Uninstall from TTY3 jumps back to the compositor.
